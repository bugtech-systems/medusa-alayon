...
let result;
    ...
    if(template.type == 'WORKFLOW'){
     ...
     result = await this.executeAction(template, context);
    }
...
console.log(config, context, "EXEC ACT", template.name)
    ...
    switch (template.type) {
      ...
      default:
        throw new Error(`Unsupported action type: ${template.type}`)
    }
  }
...
let queryConfig = {limit: 10, ...config}
  if (!this.postgresPool) {
    throw new Error("Database pool not available. Check your postgres-loader.")
  }
  ...
  try {
    console.log('Database operation config:', queryConfig)
  console.log(queryConfig, queryConfig.operation, 'query data')
    
    let queryResult: QueryBuilderResult
    let result: any
           console.log(queryConfig, 'QQUERY CONF')
    switch (queryConfig.operation) {
      case 'read':
        queryResult = this.buildSelectQuery(queryConfig)
        
        console.log(queryConfig, queryResult, 'QQUERY')
      ...
      case 'create':
        ...
        console.log(queryResult, 'queery res')
...
private quoteReservedKeywords(sql: any): string {
  ...
  private async callAI(config: ActionConfig, context: any): Promise<any> {
    ...
    console.log(config, context, 'CALL AII')
    
    let result = await generateCompletion({prompt: config.message, ...config})
    
        console.log(result, 'RESPONSE AII')

...
export async function POST(
  ...
  try {
    ...
    if(oldTemplate.parameters && oldTemplate.parameters.length){
      errors = validateActionInput(oldTemplate.parameters || [], body.parameters)
    }
  ...
  console.log("Validation failed:", errors)
   return res.status(400).json({
    ...
    if(oldTemplate.parameters){
     payload  = parseActionInput(oldTemplate.parameters || [], body.parameters)
    }
  console.log("Ready to execute:", payload, templateData, oldTemplate.name)
    
    let template = await actionEngine.execute(oldTemplate?.id, payload);
...
export async function GET(
  ...
  try {
    ...
    let template = await actionEngine.getActionTemplate(templateId);

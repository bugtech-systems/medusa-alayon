...
console.log(data, 'action data')
  ...
  const handleUpdate = (updatedAction) => {
    ...
    console.log(newConfig, 'new actionsss')
      ...
      />
          ...
          <Tabs.Content value="workflow" className="pt-6">
              <ActionWorkflowEditor
                  ...
                  actions={action?.config?.actions ? action?.config?.actions?.map(act => {
                    let actionData = actions?.actions.find(a => a.id == act.action_id);
                    if(actionData){
                     return {id: act.action_id, ...act, name: actionData.name, description: actionData.description}
                    } else {
                     return act
                    }
                }) : []}
